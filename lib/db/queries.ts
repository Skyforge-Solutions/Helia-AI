// This file contains server-side database queries only
// DO NOT import directly from client components
...
export async function createUser(email: string, password: string) {
  ...
  try {
    console.log(`Creating new user with email: ${email}, credits: 100`);
    const result = await db.insert(user).values({
      email,
      password: hash,
      credits: 100, // Ensure credits are initialized
    });
    console.log("User created successfully with 100 initial credits");
    return result;
  } catch (error) {
...
// Add new function to update user credits after purchase
export async function updateUserCredits(
  ...
  try {
    ...
    const newTotalCredits = currentCredits + additionalCredits;
